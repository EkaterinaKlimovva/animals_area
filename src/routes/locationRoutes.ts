...
import {
  ...
  geohashQuerySchema,
} from '../validation/locationSchemas';
...
router.get(
  '/geohash',
  authenticateToken,
  validate({ query: geohashQuerySchema }),
  asyncHandler(locationController.getGeohash.bind(locationController)),
);
...
router.get(
  ...
  authenticateToken,
  validate({ query: geohashQuerySchema }),
  asyncHandler(locationController.getGeohashV2.bind(locationController)),
);
...
router.get(
  ...
  authenticateToken,
  validate({ query: geohashQuerySchema }),
  asyncHandler(locationController.getGeohashV3.bind(locationController)),
);
...
router.get(
  '/',
  authenticateToken,
  asyncHandler(locationController.searchLocations.bind(locationController)),
);
...
router.put(
  ...
  authenticateToken,
  authorizeRoles(Role.ADMIN, Role.CHIPPER),
  validate({ params: locationIdParamsSchema, body: locationBodySchema }),
