...
export const latitudeSchema = z.union([
  z.string().refine((val) => {
    ...
    return !isNaN(num) && num >= COORDINATE_LIMITS.MIN_LATITUDE && num <= COORDINATE_LIMITS.MAX_LATITUDE;
  }, { message: 'Invalid latitude' }),
  z.number().refine((val) => {
    return !isNaN(val) && val >= COORDINATE_LIMITS.MIN_LATITUDE && val <= COORDINATE_LIMITS.MAX_LATITUDE;
  }, { message: 'Invalid latitude' })
]);
...
export const longitudeSchema = z.union([
  z.string().refine((val) => {
    const num = parseFloat(val);
    return !isNaN(num) && num >= COORDINATE_LIMITS.MIN_LONGITUDE && num <= COORDINATE_LIMITS.MAX_LONGITUDE;
  }, { message: 'Invalid longitude' }),
  z.number().refine((val) => {
    return !isNaN(val) && val >= COORDINATE_LIMITS.MIN_LONGITUDE && val <= COORDINATE_LIMITS.MAX_LONGITUDE;
  }, { message: 'Invalid longitude' })
]);
